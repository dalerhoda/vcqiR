...
# RI_QUAL_09_03DV R version 1.02 - Biostat Global Consulting - 2022-10-18
# *******************************************************************************
# Change log
...
RI_QUAL_09_03DV <- function(VCP = "RI_QUAL_09_03DV"){
  ...
  for (d in seq_along(MOV_OUTPUT_DOSE_LIST)) {
    ...
    dat$tempvar4 <- haven::labelled(dat$tempvar4, label = paste0("Child had a MOV on dose ", MOV_OUTPUT_DOSE_LIST[d])) %>% suppressWarnings()
    dat$tempvar5 <- haven::labelled(dat$tempvar5, label = paste0("Child had an uncorrected MOV on dose ", MOV_OUTPUT_DOSE_LIST[d])) %>% suppressWarnings()
    dat$tempvar6 <- haven::labelled(dat$tempvar6, label = paste0("Child had a corrected MOV on dose ", MOV_OUTPUT_DOSE_LIST[d])) %>% suppressWarnings()
  ...
  dat$tempvar1 <- haven::labelled(dat$tempvar1, label = "Number of doses with MOVs") %>% suppressWarnings()
  dat$tempvar2 <- haven::labelled(dat$tempvar2, label = "Number of uncorrected MOVs") %>% suppressWarnings()
  dat$tempvar3 <- haven::labelled(dat$tempvar3, label = "Number of corrected MOVs") %>% suppressWarnings()
  # had 1+ MOVs over all doses
  dat$tempvar7 <- haven::labelled(dat$tempvar7, label = "Child had 1+ MOVs over all doses") %>% suppressWarnings()
  # had only uncorrected MOVs
  dat$tempvar8 <- haven::labelled(dat$tempvar8, label = "Child only had uncorrected MOVs") %>% suppressWarnings()
  # had only corrected MOVs
  dat$tempvar9 <- haven::labelled(dat$tempvar9, label = "Child only had corrected MOVs") %>% suppressWarnings()
  # had both uncorrected and corrected MOVs
  dat$tempvar10 <- haven::labelled(dat$tempvar10, label = "Child had both corrected and uncorrected MOVs") %>% suppressWarnings()
