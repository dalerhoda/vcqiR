...
# RI_COVG_04_06PO R version 1.03 - Biostat Global Consulting - 2025-05-28
# *******************************************************************************
# Change log
...
RI_COVG_04_06PO <- function(VCP = "RI_COVG_04_06PO"){
  ...
  if (VCQI_MAKE_OP_PLOTS == 1){
    ...
    dat <- vcqi_read(paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
    ...
    if (VCQI_NO_DOBS != 1){
      vlist <- c("not_vaccinated_crude", "not_vaccinated_valid",
                 "not_vaccinated_by_age1")
    }
    ...
    for (v in seq_along(vlist)){
      ...
      for (i in 1:opp_nstrata){
        ...
        filestub <- paste0("RI_COVG_04_", ANALYSIS_COUNTER, "_opplot_",
                           abbrev, "_", id, "_", name)
  ...
  if (VCQI_MAKE_IW_PLOTS == 1){
    ...
    vcqi_to_plot(
      database = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                        ANALYSIS_COUNTER,"_nvc_database.rds"),
      filename = savepng,
      datafile = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                        ANALYSIS_COUNTER,".rds"),
      title = title_string,
      note = split_text(
        paste0(language_string(language_use = language_use, str = "OS_431"), " ",
    ...
    if (VCQI_NO_DOBS != 1){
      ...
      vcqi_to_plot(
        database = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                          ANALYSIS_COUNTER, "_nvv_database.rds"),
        filename = savepng,
        datafile = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                          ANALYSIS_COUNTER, ".rds"),
        title = title_string,
      ...
      note_string <- paste0(
        ...
        language_string(language_use = language_use, str = "OS_431"), " ",
        paste(RI_DOSES_TO_BE_FULLY_VACCINATED, collapse = ", ")
      ) |> split_text(text_cutoff = FOOTNOTE_CUTOFF)

      vcqi_to_double_plot(
        database = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                          ANALYSIS_COUNTER,"_nvv_database.rds"),
        database2 = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                           ANALYSIS_COUNTER,"_nvc_database.rds"),
        filename = savepng,
        datafile = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                          ANALYSIS_COUNTER,".rds"),
        datafile2 = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                           ANALYSIS_COUNTER,".rds"),
        title = "RI - Not Vaccinated - Valid",
      ...
      vcqi_to_plot(
        database = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                          ANALYSIS_COUNTER, "_nva1_database.rds"),
        filename = savepng,
        datafile = paste0(VCQI_OUTPUT_FOLDER, "/RI_COVG_04_",
                          ANALYSIS_COUNTER, ".rds"),
        title = title_string,
