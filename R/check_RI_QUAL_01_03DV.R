#' Check upstream output from check_RI_QUAL_01
#'
#' @param VCP VCQI current program name to be logged, default to be the function name
#'
#' @return Errors and/or warnings if conditions not met

# check_RI_QUAL_01_03DV R version 1.00 - Biostat Global Consulting - 2022-12-21
# *******************************************************************************
# Change log
...
check_RI_QUAL_01_03DV <- function(VCP = "check_RI_QUAL_01_03DV"){
  vcqi_log_comment(VCP, 5, "Flow", "Starting")

  exitflag <- 0
  errormsgs <- NULL

  filename <- paste0(VCQI_OUTPUT_FOLDER,"/RI_QUAL_01_",ANALYSIS_COUNTER,".rds")

  if (!file.exists(filename)){
    if (file.exists(paste0(VCQI_OUTPUT_FOLDER,"/RI_QUAL_01_1.rds"))) {
      file.copy(from = paste0(VCQI_OUTPUT_FOLDER,"/RI_QUAL_01_1.rds"), to = filename, overwrite = TRUE)

      if (!vcqi_object_exists("RI_QUAL_01_TEMP_DATASETS")){
        RI_QUAL_01_TEMP_DATASETS <- NULL
      }

      vcqi_global(RI_QUAL_01_TEMP_DATASETS,c(RI_QUAL_01_TEMP_DATASETS,paste0("RI_QUAL_01_",ANALYSIS_COUNTER,".rds")))
      vcqi_log_comment(VCP, 2, "Warning", paste0(filename, " does not exist. VCQI will make a copy of RI_QUAL_01_1.rds and proceed."))
    } else{
      exitflag <- 1
      errormsgs <- c(errormsgs,
                     paste0("The file", filename, " does not exist. Run RI_QUAL_01_03DV"))
      vcqi_log_comment(VCP, 1, "Error", paste0("The file ", filename, " does not exist. Run RI_QUAL_01_03DV"))
    }
