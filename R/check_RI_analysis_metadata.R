#' Check RI analysis-related globals, datasets and variables
#'
...
# 2022-06-28  1.00      Mia Yu          Original R version
# 2022-07-06  1.01      Caitlin Clary   Update log global calls, extend to all defined
#                                       multi-dose lists
# 2022-07-18  1.02      Caitlin Clary   Use vcqi_object_exists to check if any RI doses
#                                       were identified for analysis
# 2022-07-10  1.03      Mia Yu          Fixed the bug with incorrect RI data format
#                                       Use vcqi_object_exists
# 2022-08-02  1.04      Mia Yu          Added parts to call related programs
# 2022-09-20  1.05      Mia Yu          Clean up comments
# 2022-10-04  1.06      Mia Yu          Package version
# 2022-10-10  1.07      Caitlin Clary   Modify "No RI doses identified" check to
#                                       work in package context
# 2022-10-18  1.08      Caitlin Clary   Added vcqi_halt_immediately call
...
check_RI_analysis_metadata <- function(VCP = "check_RI_analysis_metadata"){
  ...
  if (VCQI_DATA_FOLDER %in% VCQI_OUTPUT_FOLDER){
    errormsgs <- "VCQI_DATA_FOLDER and VCQI_OUTPUT_FOLDER cannot be the same. Please change one path."
    vcqi_log_comment(VCP, 1, "Error",
                     "VCQI_DATA_FOLDER and VCQI_OUTPUT_FOLDER cannot be the same. Please change one path.")
  ...
  if(!vcqi_object_exists("VCQI_RI_DATASET")){
    ...
    if(file.exists(RIfile)){
      file.copy(from = RIfile, to = VCQI_OUTPUT_FOLDER, overwrite = TRUE)
    }
    if(!file.exists(RIfile)){
      errormsgs <- c(errormsgs,
                     paste0("The file defined by global macros VCQI_DATA_FOLDER/VCQI_RI_DATASET ", RIfile, " does not exist"))
      exitflag <- 1
      ...
      if(is.data.frame(dat) == FALSE){
        errormsgs <- c(errormsgs,
                       paste0("The file defined by global macros VCQI_DATA_FOLDER/VCQI_RI_DATASET ", RIfile, " is not in valid format"))
        exitflag <- 1
        ...
        result <- check_interview_date(dat = dat, errormsgs = errormsgs, exitflag = exitflag)
        errormsgs <- result$message
        exitflag <- result$flag
      }
  ...
  if(exists("MOV_OUTPUT_DOSE_LIST")){
    if (length(MOV_OUTPUT_DOSE_LIST) > 0 &
      ...
      MOV_OUTPUT_DOSE_LIST <- RI_DOSE_LIST
      MOV_OUTPUT_DOSE_LIST <- str_to_lower(MOV_OUTPUT_DOSE_LIST)
      vcqi_log_global(MOV_OUTPUT_DOSE_LIST)
    }
  }
